...
class TestScoring:
    ...
    def test_relevant_item_scores_above_threshold(self, sample_normalized_items, sample_config):
    ...
    def test_irrelevant_item_scores_below_threshold(self, sample_normalized_items, sample_config):
    ...
    def test_entity_match_boosts_score(self, sample_config):
        ...
        item_with_entity = NormalizedItem(
            item_id="ent001", source_name="Test", source_type="rss",
            source_priority=3, source_tags=[], title="UnitedHealthcare plans something",
            link="https://x.com/1", published_date=datetime(2024, 1, 1),
            summary="Generic business news about plans.",
        )
        item_without_entity = NormalizedItem(
            item_id="ent002", source_name="Test", source_type="rss",
            source_priority=3, source_tags=[], title="Some company plans something",
            link="https://x.com/2", published_date=datetime(2024, 1, 1),
            summary="Generic business news about plans.",
        )
    ...
    def test_title_keyword_weighted_higher(self, sample_config):
        ...
        item_title = NormalizedItem(
            item_id="tw001", source_name="Test", source_type="rss",
            source_priority=3, source_tags=[], title="New enrollment trends in Medicare",
            link="https://x.com/1", published_date=datetime(2024, 1, 1),
            summary="General healthcare news article.",
        )
        item_body = NormalizedItem(
            item_id="tw002", source_name="Test", source_type="rss",
            source_priority=3, source_tags=[], title="General healthcare news",
            link="https://x.com/2", published_date=datetime(2024, 1, 1),
    ...
    def test_multi_category_boost(self, sample_config):
        ...
        item = NormalizedItem(
            item_id="mc001", source_name="Test", source_type="rss",
            source_priority=3, source_tags=[],
    ...
    def test_score_clamped_to_unit(self, sample_config):
        ...
        item = NormalizedItem(
            item_id="clamp001", source_name="Test", source_type="rss",
            source_priority=5, source_tags=[],
    ...
    def test_source_priority_affects_score(self, sample_config):
        ...
        item_high = NormalizedItem(
            item_id="sp001", source_name="High Priority", source_type="rss",
            source_priority=5, source_tags=[], title="Generic news",
            link="https://x.com/1", published_date=datetime(2024, 1, 1),
            summary="Some generic news about healthcare.",
        )
        item_low = NormalizedItem(
            item_id="sp002", source_name="Low Priority", source_type="rss",
            source_priority=1, source_tags=[], title="Generic news",
            link="https://x.com/2", published_date=datetime(2024, 1, 1),
            summary="Some generic news about healthcare.",
        )
