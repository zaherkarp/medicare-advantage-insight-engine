...
| — | 2026-07-23 | #55 | **User-requested** (out of loop order): analyst-grade narrative framing. New calendar-aware synthesis lede atop the Daily Briefing (`synthesis.py` + `ma_calendar.py`) — a pure "what's happening" read over the current + prior facet windows that frames deadline-season volume as seasonal (AEP/OEP/rate-bid/Star Ratings) and flags off-cycle activity, so a routine enrollment flurry stops reading as a step-change. De-hyped the per-story drafter (`drafting.py`) to terse, fact-derived prose (removing the "evolving dynamics" / "may amplify the significance" mad-libs) with its first direct tests. `DIGEST_LEDE_ENABLED` knob (default on). | golden set untouched (no scoring/taxonomy change); 468 tests green (+28) | No schema change (reuses `get_recent_story_facets`, the same current+prior window pattern as `/angles`). Continues the #51 shift from templated hooks to fact-derived voice; `ma_calendar` dates are approximate framing aids, not compliance dates. |
